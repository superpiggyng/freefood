    ...
    <form className="stock-upload" onSubmit={analyse}>
      ...
      <section className="dashboard-panel stock-upload__result" aria-live="polite">
        ...
        {results && <>
          ...
          <button className="button button--primary button--wide" type="button" onClick={() => setPublished(included.length)} disabled={!included.length}>Publish {included.length} listing{included.length === 1 ? '' : 's'}</button>
          {published > 0 && <p className="form-success" role="status"><Check size={14}/> {published} listings published. Matched recipients in {suburb} are being notified now.</p>}
        </>}
      </section>
    </form>
