...
export function Header({ marketplace = false }: { marketplace?: boolean }) {
  ...
  return <header className="site-header">
    ...
    <div className="header-actions">
      ...
      {user ? (
        <><span className="header-user">{user.role === 'vendor' ? user.vendorName || user.username : user.username}</span><button className="button button-sm" onClick={async () => { await logout(); navigate('/'); }}>Log out</button></>
      ) : (
        <><Link className="icon-button" to="/login">Log in</Link><Link className="button button-sm" to="/register">Get started</Link></>
      )}
    </div>
  </header>;
}
